...
public:
	...
	template <typename T> T Get(uint32_t iIndex) { return StringValue::strto<T>(Get(iIndex)); }
	template <typename T> T Get(const std::string &sField) { return StringValue::strto<T>(Get(sField)); }
